...
if( class_exists('WP_Customize_Control')):
    ...
    */
    class Vmagazine_Repeater_Controler extends WP_Customize_Control {
    	/**
    	 * The control type.
    	 *
    	 * @access public
    	...
    	*/
    	...
    	private function vmagazine_get_fields(){
    		$fields = $this->fields;
    		$values = json_decode($this->value());
    		...
    		<?php
	    			...
	    			<?php 
	    				...
	    				switch ($field['type']) {
	    					...
	    					case 'upload':
	    						...
	    						esc_html_e('No image selected', 'vmagazine');
	    						echo '</div>';
	    						echo '<div class="thumbnail thumbnail-image">';
	    						echo ''.$image; //Escaping of all variables already done above
	    						echo '</div>';
	    						echo '<div class="actions clearfix">';
	    						echo '<button type="button" class="button vmagazine-delete-button align-left">'.esc_html__('Remove', 'vmagazine').'</button>';
	    						echo '<button type="button" class="button vmagazine-upload-button alignright">'.esc_html__('Select Image', 'vmagazine').'</button>';
	    					...
	    					case 'category':
	    						...
	    						echo '<option value="0">'.esc_html__('Select Category', 'vmagazine').'</option>';
	    						echo '<option value="-1">'.esc_html__('Latest Posts', 'vmagazine').'</option>';
                            ...
                            case 'page':
                                echo '<select data-default="'.esc_attr($default).'"  data-name="'.esc_attr($key).'">';
	    						echo '<option value="0">'.esc_html__('Select Page', 'vmagazine').'</option>';
	    					...
	    					case 'icon':
	    						...
	    						$vmagazine_icons_array = vmagazine_icons_array();
	    					...
	    					case 'social_icon':
	    						...
	    						echo '</div>';
	    						echo '<ul class="vmagazine-icon-list clearfix">';
	    						$vmagazine_icons_array = vmagazine_social_icons_arrays();
	    						foreach ($vmagazine_icons_array as $vmagazine_font_awesome_icon) {
	    					...
	    					case 'multicategory':
	    						...
	    						echo '<li><label><input type="checkbox" value="-1" '. checked('-1', $new_value, false ) .'/>'.esc_html__( 'Latest Posts', 'vmagazine' ).'</label></li>';
	    						foreach ( $this->cats as $cat ){
    			...
    			<div class="clearfix vmagazine-repeater-footer">
    				...
    				<a class="vmagazine-repeater-field-remove" href="#remove"><?php esc_html_e('Delete', 'vmagazine') ?></a> |
    				<a class="vmagazine-repeater-field-close" href="#close"><?php esc_html_e('Close', 'vmagazine') ?></a>
    				</div>
    			</div>
    		</div>
    		</li>
