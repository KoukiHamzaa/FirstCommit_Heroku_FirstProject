...
<div class="ap-settings-wrapper wrap">
    ...
    <?php if ( isset($_GET[ 'message' ]) ) {
        switch($_GET['message']){
            case 1:
            $message = __('Form updated Successfully.', 'anonymous-post-pro');
            break;
            case 2:
            $message = __('Default Settings restored successfully.', 'anonymous-post-pro');
            break;
    ...
    if ( isset($_GET[ 'form_id' ]) ) {
        ...
        <div class="ap-pro-shortcode-ref">
            <h3>Shortcode:</h3>
            <input type="text" readonly="readonly" value='[ap-form id="<?php echo intval($_GET[ 'form_id' ]) ?>"]' onfocus="this.select();">
            <a href="<?php echo site_url() . '/?ap_preview=true&form_id=' . intval($_GET[ 'form_id' ]); ?>" target="_blank">
    ...
    <ul class="ap-settings-tabs">
        ...
        <li><a href="javascript:void(0)" id="morewp-settings" class="ap-tabs-trigger"><?php _e('More WordPress Resources', 'anonymous-post-pro'); ?></a></li>
    </ul>
            ...
            <form class="ap-settings-form" method="post" action="<?php echo admin_url() . 'admin-post.php' ?>">
                ...
                <div id="optionsframework-submit" class="ap-settings-submit">
                    ...
                    <?php if ( isset($_GET[ 'form_id' ]) ) { ?>
                        ...
                        <a href="<?php echo admin_url() . '/admin-post.php?action=ap_restore_default_settings&form_id=' . intval($_GET[ 'form_id' ]) . '&_wpnonce=' . $restore_nonce; ?>">
                ...
                <?php if ( isset($_GET[ 'form_id' ]) ) { ?>
                    <input type="hidden" value="<?php echo intval($_GET[ 'form_id' ]); ?>" name="ap_form_id"/>
                <?php } ?>
            </form>
