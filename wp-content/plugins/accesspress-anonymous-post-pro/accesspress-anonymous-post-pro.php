...
//                    {
        ...
        function ap_form_message($atts) {
            global $ap_success_message;
            if (!empty($ap_success_message)) {
                
                return $ap_success_message;
            } else {
        ...
        function ap_pro_restore_default_settings() {
            ...
            if (!empty($_GET) && wp_verify_nonce($nonce, 'ap-restore-default-nonce')) {
                ...
                wp_redirect(admin_url() . 'admin.php?page=anonymous-post-pro&action=edit_form&form_id=' . $form_id.'&message=2');
                exit;
            } else {
                die('No script kiddies please');
            }
        ...
        function ap_form_delete_action() {
            if (isset($_GET['_wpnonce']) && wp_verify_nonce($_GET['_wpnonce'], 'ap-form-delete-nonce')) {
               ...
               // } else {
                ...
                wp_redirect(admin_url() . 'admin.php?page=anonymous-post-pro&message=2');
                exit;
            } else {
                die('No script kiddies please!');
            }
        ...
        function extra_settings_action() {
            if (!empty($_POST) && wp_verify_nonce($_POST['ap_extra_setting_nonce_field'], 'ap_extra_setting_nonce')) {
                ...
                wp_redirect('admin.php?page=ap-pro-settings&message=1');
                exit();
            } else {
                die('No script kiddies please!!');
            }
        ...
        function check_cookie(){
            if ( isset($_COOKIE[ 'ap_form_success_msg' ], $_COOKIE[ 'ap_form_id' ]) ) {
                $GLOBALS['ap_success_message'] = $_COOKIE[ 'ap_form_success_msg' ];
                $GLOBALS['ap_form_id'] = $_COOKIE[ 'ap_form_id' ];
                unset($_COOKIE[ 'ap_form_success_msg' ]);
                unset($_COOKIE[ 'ap_form_id' ]);
               setcookie( 'ap_form_success_msg', '', time() - ( 15 * 60 ), COOKIEPATH, COOKIE_DOMAIN );
                       
                setcookie( 'ap_form_id', '', time() - ( 15 * 60 ), COOKIEPATH, COOKIE_DOMAIN );
               
            }else{
                $GLOBALS['ap_success_message'] = '';
            }
        }
