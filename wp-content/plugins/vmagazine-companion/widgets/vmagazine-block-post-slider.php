...
class vmagazine_block_post_slider extends WP_Widget {
    ...
    public function __construct() {
        $widget_ops = array(
            ...
            'description' => __( 'Display posts from selected category as tabbed slider.', 'vmagazine-companion' ),
        );
        $width = array(
                'width'  => 600
        );
        parent::__construct( 'vmagazine_block_post_slider', esc_html__( 'Vmagazine : Block Post Slider', 'vmagazine-companion' ), $widget_ops,$width );
    }
            ...
            <div class="block-header clearfix">
                <?php
                    ...
                    foreach ( $vmagazine_block_multi_cats as $key => $term_id ) {
                        ...
                        if(!empty( $term )){
                            echo '<li><a href="javascript:void(0)" data-meta="'.esc_attr($block_section_meta).'" data-id="' . intval( $key ) . '" data-slug="' . intval( $key ) . '" data-link="'.get_term_link($term->slug, 'category').'" data-offset="'.absint($vmagazine_block_posts_count).'">' . $term->name . '</a></li>';
